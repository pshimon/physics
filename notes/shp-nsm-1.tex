...
First, we formulate the physical problem independently of any
particular mathematical formalism.

Second, we review the principal obstacles that have prevented previous
attempts from achieving a complete relativistic stochastic theory.

Third, we explain why these obstacles naturally lead to the SHP
...
First, we review the existing theoretical framework, including SHP
...
Second, we perform a critical comparison of these theories in order to
identify the precise obstacles that prevent their unification.

Finally, building upon this analysis, we develop a stochastic extension
...
First, proper time exists only along timelike trajectories. It cannot
parameterize null worldlines and becomes problematic in processes
involving virtual or off-shell particles.

Second, proper time depends upon the trajectory itself. Two interacting
...
Finally, proper time is not an independent dynamical variable. It is
...
of affine parameter merely correspond to different parameterizations of
the same trajectory.
...
The particle trajectory is usually parameterized by the proper time or by
...
be merely a reparameterization of conventional relativistic mechanics.
...
First, the evolution parameter is independent of the trajectory.

Second, Hamiltonian evolution is generated by an ordinary scalar
Hamiltonian rather than by a constraint.

Third, the temporal coordinate is treated as a canonical dynamical
variable on the same footing as the spatial coordinates.

Finally, the formalism naturally accommodates interacting many-particle
...
dx^\mu
=
\frac{\partial K}{\partial p_\mu}\,d\tau
+
d\xi^\mu,
\]
...
=
-
\frac{\partial K}{\partial x^\mu}\,d\tau
+
d\eta_\mu,
\]
