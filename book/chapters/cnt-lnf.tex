...
trajectories or over instantaneous velocities --- reproduces the structure of
quantum mechanics.


What the standard presentation quietly elides is that ``$\delta S=0$'' is a
...
piecewise-$C^1$ or $C^2$ curves from the outset. 
...
Lavrentiev observed in 1926 that the infimum of a variational integral can
    ...
    example (1934):
\[
I[u] \;=\; \int_0^1 \bigl(u(x)^3-x\bigr)^2\, \dot u(x)^6\, dx, 
    \qquad u(0)=0,\ u(1)=1.
\]
...
\paragraph{Relaxation: Young measures as generalized solutions}
 When $v\mapsto L(t,q,v)$ fails to be convex, a
...
\item Hamilton's principle, as usually taught, silently restricts the
    admissible trial paths to a class (piecewise-$C^1$, or $W^{1,2}$ upgraded
        ...
        accompanying regularity theory are the actual content behind 
        ``$\delta S=0 \Rightarrow$ Newton's equation.''
...
\end{enumerate}
Points 3 and 4 will be discussed in details later in this book.
