...
\input{package}
\usepackage{showkeys} %show labels, useful for draft
\input{nt}
\input{box}
\input{cmd}
...
which frequently arise on studying theoretical physics but rarely answered.
 ...
 I actively use AI (free versions of Gemini, ChatGPT and Claude) especially when 
 talking about things beyond my professional background like philosophy of 
 physics or abstract mathematics. I always check the references but never 
 include those which I have never read in the book. The reader may see 
 sometimes sudden change of writing style when I combine my own text 
 with that produced by AI. However I carefully edit all equations so
 errors (hopefully rare enough) are mistypes not careless copy from AI.
...
\mainmatter
\part{Metaphysics}
%\begin{refsection}    %partial bibliography
\input{chapters/mp-models}
\input{chapters/mp-limits}
%\printbibliography[heading=subbibliography] %partial bibliography
%\end{refsection}%partial bibliography
...
\part{Classical Nonrelativistic Theory}
\input{chapters/cnt-intro}
\input{chapters/cnt-chs}
\input{chapters/cnt-vp}
\part{Classical Relativistic Theory}
\part{Quantum Nonrelativistic Theory}
\part{Quantum Relativistic Theory}
\backmatter
%\nocite{*} %prints full database, not only cited entries
\printbibliography[
heading=bibintoc,
title={Bibliography}
] 
