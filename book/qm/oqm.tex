...
Formally a quantum system is defined by the following axioms,
describing states,observables,dynamics, and probabilities.
\begin{axiom}[States]
    \label{cq:sts}
    Physical state is a (non-zero) vector in complex separable Hilbert space $ \mathcal{H} $
...
and restrict \eqref{oqm:pcc} to these variables, 
...
Operator $ \hat H$ (Hamiltonian) generates time evolution (unitary group):
 ...
 \hat U(t) = e^{-i \hat H t/ \hbar}.
    ...
    Evolution may be seen as action on observables (Heisenberg picture)
\begin{align}
   \hat A(t)&= \hat U(t) \hat A \hat U^{-1}(t), \label {oqm:heis1}\\
    \frac{d \hat A}{d t} &= \frac{i}{\hbar}[\hat H,\hat A],\label{oqm:heis2}
\end{align}
    or as action on states (Schr\"odinger picture)
\begin{align}
    |\psi(t)\rangle&= \hat U^{-1}(t) |\psi \rangle, \label{oqm:schr1} \\
    \frac{d |\psi \rangle}{d t} &=-\frac{i}{\hbar} \hat H |\psi \rangle. \label{oqm:schr2}
\end{align}
...
In particular using \eqref{oqm:ham},\eqref{oqm:ccr} and \eqref{oqm:heis2} we get:
\begin{equation}
   \label {oqm:ehrenfest}
   \begin{split}
...
\begin{remark}
    ...
    canonical commutation relations \eqref{oqm:ccr} is unbounded.
\end{remark}  
